...
void CWebBrowser2::Init()
{
	CString fn(_T(""));
	::GetModuleFileName(NULL, fn.GetBuffer(MAX_PATH), MAX_PATH);
	fn.ReleaseBuffer();
	fn = fn.Left(fn.ReverseFind(_T('\\')) + 1);
	CString url(_T("file:///"));
	fn.Replace(_T("\\"), _T("/"));
	url.Append(fn);
	url.Append(_T("BaiDuIndex.html"));
	Navigate(url, NULL, NULL, NULL, NULL);
}

void CWebBrowser2::PutPiont(double lng, double lat)
{
	CWebPage web;
	web.SetDocument(GetDocument());	
	const CString funcName("putpoint");	//JS函数名
	CComVariant varResult;	
	CString strLng, strLat;
	strLng.Format(_T("%f"), lng);
	strLat.Format(_T("%f"), lat);
	web.CallJScript(funcName, strLng, strLat,&varResult);
}

void CWebBrowser2::ClearMarkers()
{
	CWebPage web;
	web.SetDocument(GetDocument());	
	const CString funcName("clearMarkers");	//JS函数名
	CComVariant varResult;	
